# SEED: mypeople-03-lifecycle

> seed-format: 1

## Goal

Extend the core agent loop (`SEED 2`) with **lifecycle notifications + Boss-as-role**:

1. **Lifecycle notifications (the plumbing).** When a spawned agent finishes a turn (Stop event): a JSON status file is written to `~/mypeople/status/<session>/<agent>.json`, and — if spawned with `--boss <boss_id>` — an `[AGENT NOTIFICATION]` line is typed into the boss's tmux pane via the queue. Reuses `/task/submit`; no new endpoints.
2. **Boss-as-role (the behavior).** `mp spawn --master <agent_id>` installs the Boss doctrine (`boss-CLAUDE.md`) into the container at install time AND, immediately after the spawned claude reaches its prompt, sends an onboarding message that has the agent read its job description. The "Boss" is not just a tab — it's an agent that has internalized plan-gate + autonomous-loop + fire-and-forget.

Verification tests both layers: transport (status file + notification routing) AND role behavior (Boss summary after onboarding mentions doctrine keywords; Boss can orchestrate a worker end-to-end with `mp` and report the worker's result back).

This seed is a **superset** of SEED 2.
...
# Truncate summary to 200 chars, single-line
...
# Cleanup
mp kill "main:worker-1" >/dev/null || true
mp kill "main:Boss" >/dev/null || true
